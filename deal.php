...
<head>
      <meta charset="utf-8" />
      <title>Indus-Coupons | Deals</title>
            ...
            <div class="container">
               <div class="row">
                                ...
                                <form class="form-horizontal select-search" method="post">
                                    ...
                                    <div class="btn-group" data-toggle="buttons">
                                        <label class="btn btn-default active"> <i class="ti-tag"></i>
                                            <input type="checkbox" checked="">Deals</label>
                  ...
                  <!--/col -->
                              ...
                              $sel=mysqli_query($conn," SELECT deals.id, deals.date,deals.exp_date,deals.discription,deals.link,deals.image,deals.deal_code, store.logo, store.name,store.website,store.content,category.name
                              from store join deals on store.id=deals.store JOIN Category on category.id=deals.category where deals.exp_date>='".$today_date."'");
                              $num=mysqli_num_rows($sel);
                              for($i=1; $i<=$num; $i++)
                              {
                                    $row=mysqli_fetch_array($sel);
