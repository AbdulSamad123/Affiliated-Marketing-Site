        ...
        <div class="breadcrmb-wrap hidden-xs">
            ...
            <div class="container">
                <div class="row">
                    ...
                    <?php 
                    $conn=$_SESSION['conn'];
                    $str = $_SESSION['coupon.id'];
                    $select=mysqli_query($conn,"SELECT coupon.id, coupon.date, coupon.exp_date, coupon.discription,coupon.link, coupon.coupon_code, store.logo, store.name, store.website, store.content,category.name
                    FROM store JOIN coupon on store.id=coupon.store JOIN Category on category.id=coupon.category where store.id='".$str."'");
                    ...
                    for($i=1; $i<=$num; $i++)
                    {
                          ...
                          echo '<div>';
                          echo '<div class="media-center media-middle "> <img src="assets/images/'."$row[6]".'" style=="height:64px;width:64px;"> </div>';
