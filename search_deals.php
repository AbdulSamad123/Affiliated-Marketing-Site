            ...
            <div class="container">
               <div class="row">
                  ...
                  <!--/col -->
                              ...
                              <?php 
                              $conn=$_SESSION['conn'];
                              $str = $_SESSION['deals.id'];
                              $sel=mysqli_query($conn,"SELECT deals.id, deals.date, deals.exp_date, deals.discription,deals.link,deals.image, deals.deal_code, store.logo, store.name, store.website, store.content,category.name FROM store JOIN deals on store.id=deals.store JOIN Category on category.id=deals.category where store.id='".$str."'");
                              $num=mysqli_num_rows($sel);
                              for($i=1; $i<=$num; $i++)
                              {
                                    $row=mysqli_fetch_array($sel);
